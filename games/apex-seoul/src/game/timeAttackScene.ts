...
import {
    BURNOUT_PUFF_KEYS,
    CITY_FAR_LIGHTS_KEY,
    CITY_FAR_PARALLAX_KEY,
    CITY_RIDGE_PARALLAX_KEY,
    CLOUD_DARK_BLUE_KEY,
    FT86_RETRO_SPRITE_URLS,
    ft86RetroShadowSpriteUrl,
    genesisG70VehicleShadowSpriteUrl,
    genesisG70VehicleSpriteUrl,
    miraeGtPreview192ShadowSpriteUrl,
    miraeGtPreview192SpriteUrl,
    MOON_COOL_BLUE_KEY,
    MIRAE_GT_SPRITE_URLS,
    RAVEN_COUPE_SPRITE_URLS,
    ravenCoupePreview192ShadowSpriteUrl,
    ravenCoupePreview192SpriteUrl,
    ravenCoupePreview256ShadowSpriteUrl,
    ravenCoupePreview256SpriteUrl,
    seorinGtPreview192ShadowSpriteUrl,
    seorinGtPreview192SpriteUrl,
    SEORIN_GT_SPRITE_URLS,
    WALL_FOREST_TREE_KEYS,
} from './startupAssetManifest';
import {
...
import { type VehicleRoadScaleConfig } from './vehicleRoadScale';
import { resolveRunSetup, type RunSetup } from './runSetup';
import { selectRuntimeVehicleAsset, type RuntimeVehicleSelection } from './vehicleCatalog';
import {
...
const FINISH_COAST_DURATION_SEC = 1.25;
const FINISH_SUMMARY_DURATION_SEC = 2.25;
...
export class TimeAttackScene extends Phaser.Scene {
    ...
    private finishPresentationPhase: 'racing' | 'capture' | 'coast' | 'finish-summary' | 'results' = 'racing';
    private finishCoastPresentation: FinishCoastPresentation | null = null;
    private finishSummaryRemainingSec = 0;
    ...
    update(_time: number, delta: number) {
        ...
        if (this.runState.finished) {
            if (this.finishPresentationPhase === 'results') {
            ...
            if (this.finishPresentationPhase === 'capture') {
                ...
                if (this.finishCoastRemainingSec === 0) {
                    this.finishPresentationPhase = 'finish-summary';
                    this.finishSummaryRemainingSec = FINISH_SUMMARY_DURATION_SEC;
                }
            } else if (this.finishPresentationPhase === 'finish-summary') {
                this.finishSummaryRemainingSec = Math.max(0, this.finishSummaryRemainingSec - seconds);
                if (this.finishSummaryRemainingSec === 0) this.finishPresentationPhase = 'results';
            }
            camera.pitch = this.updateCameraPitch(seconds);
            this.updateSpeedEffect(seconds);
            camera.fovDegrees = this.cameraEffects.fovDegrees;
    ...
    private render(seconds = 0) {
        ...
        if (this.finishPresentationPhase === 'finish-summary') {
            this.uiGraphics.fillStyle(0x010305, 0.68);
    ...
    private getFinishCoastVehicleFade() {
        if (this.finishPresentationPhase === 'finish-summary' || this.finishPresentationPhase === 'results') return 0;
    ...
    private updateRestartHotkey() {
        if (this.runState.finished) return;
    ...
    private updateRunState(seconds: number) {
        ...
        if (finishedNow) {
            ...
            this.finishSummaryRemainingSec = 0;
...
export const APEX_SEOUL_GAME_CONFIG: Phaser.Types.Core.GameConfig = {
