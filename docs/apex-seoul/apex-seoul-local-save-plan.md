...
## 2026-09-13 Records 단순화 반영

이 항목이 앞선 전체/차량별·베스트/최근 탭 설계를 대체한다. Records는 마지막 선택 코스·현재 규칙의 최근 완주를 차량 구분 없이 합쳐 `finishedAt` 내림차순으로 표시한다. 동시각은 runId 순으로 고정하고 중복 runId는 한 번만 표시한다. 빠른 완주 시간순 랭킹이 아니므로 순위 번호는 붙이지 않는다.

화면에는 **차량 스프라이트 / 완주 시간 / 이름** 세 열과 페이지 이동·메뉴 복귀만 둔다. 페이지당 5개이며 현재 bucket별 최근 20개 보관은 유지한다. PB·통계·색상·복귀 횟수·legacy 요약·탭·차량/코스 선택은 이 화면에서 제거했다. 추가 코스 선택은 기존 후속 garage 선택 UI가 담당한다.
