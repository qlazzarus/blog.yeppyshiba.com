...
## 2026-09-17 Records 단순화 반영

이 항목이 앞선 전체/차량별·베스트/최근 탭 설계를 대체한다. Records는 마지막 선택 코스·현재 규칙의 최근 완주를 차량 구분 없이 합쳐 완주 시간 오름차순으로 표시한다. 동률은 `finishedAt` 오름차순, 이어서 runId 순으로 고정하고 중복 runId는 한 번만 표시한다. 이는 PB 비교와 같은 정렬 규칙이다.

화면에는 **차량 스프라이트 / 완주 시간 / 이름** 세 열만 둔다. 페이지 이동은 없으며, 화면을 넘는 기록은 마우스 휠로 같은 목록을 스크롤한다. 키보드의 어떤 키든 즉시 메인 메뉴로 돌아간다. 현재 bucket별 최근 20개 보관은 유지한다. PB·통계·색상·복귀 횟수·legacy 요약·탭·차량/코스 선택은 이 화면에서 제거했다. 추가 코스 선택은 기존 후속 garage 선택 UI가 담당한다.

`RunSummary.playerName`을 호환 가능한 선택 필드로 추가했다. 입력 UI가 없는 현재는 저장·읽기 시 누락값을 `PLAYER`로 보정한다. 향후 세 글자 이름을 위해 `A-Z` 정확히 세 글자와 기본값 `PLAYER`만 허용한다. 기존 기록은 삭제하지 않으며 JSON schema와 주행 ruleset은 이번 UI 변경으로 올리지 않는다.

전체/현재 차량의 최고 완주와 해당 run의 checkpoint 누적 시간은 별도로 유지한다. 주행 시작 snapshot과 LAST SPLIT·결승의 두 기준 비교는 변경하지 않는다. 이름·차량·시간 외에도 내부적으로 finishedAt, runId, trackId, rulesetVersion, checkpointTimesSec 등 비교와 저장에 필요한 데이터를 유지한다.

### TODO — 경기 종료 후 영문 세 글자 이름 입력

사용자 요청으로 문서에만 남긴 후속 작업이며 이번에는 구현하지 않는다.

- 정상 경기 종료 후 이름 입력 단계를 제공한다. 정확히 세 글자, 각 자리는 a-z만 입력 가능하며 저장·화면에는 대문자 A-Z로 통일한다. 현재 임시 이름 `PLAYER`는 입력 선택지에 포함하지 않는다.
- 키보드와 클릭/터치로 각 글자 선택, 자리 이동, 수정, 확정을 지원한다. 마지막 확정 이름을 다음 경기의 초기값으로 복원한다.
- 현재 완주는 결과 진입 전에 저장되므로 이름 확정 시 동일 runId의 이름만 갱신한다. recentRuns와 해당 run이 PB일 때 bestRun의 이름을 함께 바꾸되 완주 횟수·시간·PB 판정은 다시 집계하지 않는다.
- 입력을 건너뛰거나 탭을 닫아도 이미 확정된 완주 기록을 잃지 않게 한다. 미확정 이름은 현재 기본값으로 유지하고 저장 실패도 기존 정책을 따른다.
- 검증: 세 글자 제한/숫자·공백·특수문자 거부, 소문자 정규화, 수정·확정·재진입, PB와 이력 이름 일치, 중복 집계 방지, 새로고침·저장 실패.

Records 차량 표시는 차량명 대신 기존 production sheet의 앞 3/4 프레임 6을 좌우 반전한 왼쪽 전방 스프라이트를 사용한다. 기록의 차량 ID와 색상을 반영하고, 유효하지 않은 과거 색상은 blue로 보정한다. 새 이미지 자산·저장 형식 변경 없이 기존 로딩 자산을 재사용한다.
