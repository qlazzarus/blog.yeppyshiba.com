...
- "약한 타워에게 맞으면 그 타워를 친다"는 별도 일반 규칙은 Warsmash/JASS 관찰 범위에서 확인되지 않았다.
- 정적 JASS에서 확인된 강제 포커스는 큰 개 low HP taunt, 특수 보스/수동 탱커 표식처럼 특정 이벤트성 흐름이다.
- 원본 체감상 약한 타워가 어그로를 끄는 것처럼 보이는 현상은 attack-move 중 자동 획득, 최근 피격/주변 공격 가능 대상 전환, 플레이어가 만든 미로 배치가 겹친 결과로 해석하는 편이 안전하다.
- Phaser PoC에서는 이 체감을 검증하기 위해 "최근 나를 공격한 타워를 임시 포커스로 삼고, 짧은 lock 이후 다른 타워 피격으로 재전환 가능"한 축약 규칙을 둔다.
...
Warsmash `CUnit`의 생산 완료 흐름은 건물 위치에 유닛을 생성하고,
`trainedUnit.nudgeAround(game, structure)`의 `setPointAndCheckUnstuck()`로 64
world-unit 간격의 유효 위치를 찾은 뒤, rally point가 있으면 생산 유닛에
`OrderIds.smart` 명령을 발행한다.

Chicken Farm은 이를 다음 경계로 축약한다.

- `BuildingTemplateConfig.productionExit`: 선호 출입구 방향, 측면 offset, clearance.
- `resolveBuildingProductionExit()`: footprint와 유닛 반경으로 출구를 계산하고 막히면 다른 측면/바깥 ring을 탐색한다.
