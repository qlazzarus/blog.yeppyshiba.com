...
알은 판매하거나 부화시킬 수 있는 필드 item/object다. 골드/나무처럼 HUD에 숫자로만 누적되는 wallet resource가 아니라, 맵에 드롭되고 농부가 수집하거나 닭장에서 소비하는 대상으로 취급한다.
...
타워 사거리는 원본 W3X 수치를 그대로 런타임에 쓰지 않는다. 원본 값은 분석 기준으로 보존하고, Phaser 런타임에서는 `32px` minor tile 위에서 세로 `9` major tile 전후의 농장 배치 화면을 기준으로 체감 사거리를 별도 관리한다. 오른쪽 월드 영역을 살리기 위해 가로 시야는 `16` major tile까지 허용한다.

| MVP id | 원본 rawcode | 원본 사거리 | Phaser 런타임 사거리 | minor tile | major tile | 의도 |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| `tower_scout` | `h00D` | 650 | 384px | 12.0 | 3.0 | 초반 안전 타워. 후보: 384=대각 펜스 최소, 448=타워-빈칸-펜스 이후 약간 커버, 512=꺾인 방어선 커버, 650=원본 |
| `tower_guard_small` | `h007` | 750 | 225px | 7.03 | 1.76 | 저렴한 일반 공격 업그레이드 |
| `tower_guard_medium` | `h00Q` | 850 | 255px | 7.97 | 1.99 | 중반 일반 타워 |
| `tower_guard_large` | `h00Z` | 950 | 285px | 8.91 | 2.23 | 중반 이후 방어선 강화 |
| `tower_guard_grand` | `h00R` | 1050 | 315px | 9.84 | 2.46 | 후반 일반 타워 |
| `tower_guard_black` | `h01J` | 1150 | 345px | 10.78 | 2.70 | 최상위 일반 타워 |
| `tower_arcane_small` | `h016` | 900 | 270px | 8.44 | 2.11 | 비싸지만 빠른 특수 타워 시작 |
| `tower_arcane_medium` | `h008` | 1000 | 300px | 9.38 | 2.34 | 중반 특수 타워 |
| `tower_arcane_large` | `h017` | 1100 | 330px | 10.31 | 2.58 | 후반 특수 타워 |
| `tower_arcane_grand` | `h01K` | 1200 | 360px | 11.25 | 2.81 | 최상위 특수 타워 |
...
MVP 기준 wallet resource는 골드/나무로 시작하고, 알은 별도 field item으로 관리한다.

| 자원 | 용도                           |
| ---- | ------------------------------ |
| 골드 | 닭 구매, 타워 건설, 업그레이드 |
| 나무 | 울타리, 타워, 수리             |
| 알   | field item. 수집 후 판매 또는 닭장 부화에 사용 |
...
|    7 | 닭/알/수익 PoC           | 닭 배치, field egg 생산, 알 수집/판매/부화, 30초 egg drop tick이 실제 운영 선택을 만드는지 확인                                  | chicken/economy systems                                                                                       |
...
| 항목            |                        값 | 이유                                                |
| --------------- | ------------------------: | --------------------------------------------------- |
| Canvas          |                 960 x 720 | 960 x 540 world viewport와 180px 하단 UI를 함께 둠 |
| World scale     |                        2x | 16px 타일을 32px처럼 보여 워3 grid 감각에 가깝게 함 |
| Effective world |             10080 x 10528 | 원본 pathing 기준 불필요한 상단/좌우 타일을 trim한 크기 |
| Camera viewport |                  960 x 540 | 오른쪽 월드 영역을 살리고 세로 `9` major tile을 맞춤 |
| Camera zoom     |                   0.46875 | 32px minor 기준 `64 x 36`, 128px major 기준 `16 x 9` 표시 |
| Camera speed    |              360~520 px/s | 방향키 이동이 너무 느리지 않게 시작                 |
| Farmer speed    |                240 px/sec | 원본 `H000` 농부 이동속도 240과 1:1 감각으로 맞춤   |
| Timber wolf speed |              330 px/sec | 원본 `n007` 팀버 울프 이동속도 330과 1:1 감각으로 맞춤 |
...
- WASD/방향키는 플레이어 직접 이동이 아니라 camera pan으로 사용한다. 현재 기준 속도는 `440 px/s`이며, 문서의 Warsmash 참고 범위 `360~520 px/s` 안에 둔다.
- 숫자키 `1~8`로 debug player marker와 camera focus를 시작 슬롯으로 즉시 전환할 수 있다.
- 실제 플레이어 이동속도는 Player Command Control PoC에서 원본 `H000` 농부 `240`, `n002` 개 `290` 기준으로 command/path follow 방식에 반영한다.
- 늑대 이동속도는 원본 `n007` 팀버 울프 `330` 기준으로 맞췄다. 기존 `108~115 px/s`는 축약 PoC/측정용 값으로 더 이상 사용하지 않는다.
- Warsmash 기준으로 전투 중 target이 사거리 밖으로 이동하면 공격자는 target 쪽으로 이동해 다시 사거리 안으로 들어가야 한다. 현재 PoC 늑대도 농부/개를 획득한 뒤에는 range leash/repath 기준으로 추격한다.
- 카메라 zoom은 `0.46875`로 두고, PoC 월드 뷰포트는 `960 x 540`으로 둔다. 이렇게 하면 `32px` minor 기준 `64 x 36`, `128px` major 기준 `16 x 9`가 한 화면에 보인다.
- 16px tile x 2 scale 기준 32px minor grid를 표시하되, 4 minor마다 major line을 더 굵고 진하게 표시한다. 플레이어의 배치 판단은 major grid를 기준으로 한다.
- 전투 PoC 오브젝트는 32px minor cell에 snap한다. 임시 footprint는 펜스 `4x4` minor 또는 `1x1` major, 타워 `4x4` minor 또는 `1x1` major, 농장 목표는 후속 건물별 major footprint 기준으로 조정한다.
- 펜스 segment 반복 배치는 `{ id, x, y }` 좌표를 직접 나열한다. 원본 `h003`/벽 계열의 `pathTex=PathTextures\4x4SimpleSolid.tga` 근거에 맞춰 실제 fence blocker는 `4x4` minor footprint를 따른다.
- 전투 PoC 배치는 원본 W3X의 넓은 농장/우회 감각을 확인하되, 화면 기준은 워3 실제 플레이 이미지의 세로 `9` major tile 경험을 우선한다. 가로는 브라우저 뷰포트를 살려 `16` major tile까지 표시한다.
- 현재 전투 PoC 배치는 `COMBAT_POC_LAYOUT`의 `originOffsetCells`, `fences`, `stoneRows`를 수정해 조정한다. 펜스는 반복 범위가 아니라 `{ id, x, y }` 좌표 하나가 `4x4` minor blocker 하나를 의미한다.
