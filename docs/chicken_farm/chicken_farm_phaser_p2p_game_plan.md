...
MVP 기준 목표 플레이 타임은 10~15분이다.

장기 목표는 20~30분까지 확장 가능하지만, 첫 버전에서는 친구끼리 가볍게 반복 플레이 가능한 짧은 세션을 우선한다.
...
MVP에서는 각 플레이어 농장을 완전히 분리하고, 늑대가 각 농장 방향으로 따로 이동하도록 하는 것이 구현이 쉽다.
...
| 파일 | 용도 |
|---|---|
| `docs/chicken_farm_w3x_artifacts/web_mvp_balance_reference.json` | 원본 참조값과 MVP 변환값을 나란히 기록한 근거 파일 |
| `games/chicken-farm/src/game/balance.ts` | Phaser 런타임에서 import할 타입/상수 데이터 |
| `docs/chicken_farm_wave_shop_disease_mvp_spec.md` | 웨이브/상점/질병 구현 단위 스펙 |
...
10~15분 뒤 결과창에서 누가 가장 잘 버텼는지 확인한다.
```

이 게임의 성공 기준은 그래픽 완성도가 아니라, **친구들이 한 판 더 하자고 말할 정도의 짧고 강한 운영 압박감**이다.
