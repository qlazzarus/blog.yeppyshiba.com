...
- 현재 PoC 맵은 원본 `.wpm` pathing 크기와 같은 352 x 336 tiles, 16px tilesheet 기준 5632 x 5376px이다.
...
| Effective world |             11264 x 10752 | 원본 `.wpm` pathing world와 같은 크기                |
...
- 전투 PoC 배치는 원본 W3X의 넓은 농장/우회 감각을 확인하기 위해 P6 기준 최소 20 cell 이상의 폭을 사용한다.
- 현재 전투 PoC 배치는 `FarmScene.createCombatPoc()`의 `fenceRows`, `fenceColumns`, `fenceSingles`, `stoneRows` 배열을 수정해 조정한다.
...
W3X 분석 결과 기준으로 terrain blocker를 Phaser Tilemap 또는 별도 pathing grid에 이식하는 것은 가능하다. 현재 PoC Tilemap은 원본 `.wpm` pathing 크기와 같은 352 x 336 tile로 확장했다.
...
| 현재 Phaser Tilemap     | 352 x 336 tiles, 16px tile          | 원본 pathing cell 수와 동일               |
| 현재 Phaser 표시 world  | 11264 x 10752px, `defaultScale: 2`  | 원본 pathing world와 동일                 |
| 크기 차이               | 없음                                | 시각 타일 16px를 2배 표시해 32px pathing cell과 맞춤 |
