...
완료된 기반 PoC의 압축 이력은
[`archive/completed_poc_history.md`](./archive/completed_poc_history.md)에 보관한다.
이 섹션은 현재 active planning 중 필요한 빠른 참조로만 유지한다.
...
- WolfAI decision table `6/6` 통과, Warsmash fit checks `9/9` 통과
...
| ---------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 스폰/기본 목표   | spawn entry에서 local farm/play area 랜덤 지점으로 attack-move order 발행                                  | 원본 `IliiIII`의 `GetRandomLocInRect(IlIlI)` attack 명령을 PoC 좌표계로 축약                            |
| 일반 acquisition | 농부/개 유닛과 non-fence targetable building은 acquire range 안에서 focus/attack 가능                      | Warsmash식 auto acquire와 부합                                                                          |
...
| 목표 후 cleanup  | 별도 cleanup phase는 비활성. 남은 fence 파괴는 attack-move 경로 실패/blocker fallback의 결과로만 발생      | 타운홀 파괴 후 청소 모드가 아니라 랜덤 attack 갱신과 pathing/acquisition 결과로 보는 편이 원본에 가까움 |
| 타워 피격 focus  | 맞은 늑대와 주변 assist 늑대 일부에 짧은 tower focus pulse 적용                                            | 일반 규칙으로 과도한 전체 공유 어그로는 피하고, 사용자 체감의 attacked-event group pull 일부만 축약     |
...
| dead 상태        | 사망 늑대 action/path/focus/target 정리                                                                    | 죽은 늑대에 focus를 재부여하지 않음                                                                     |
| global refresh   | 60초 주기 랜덤 attack-move refresh와 3.5초 stuck refresh 적용                                              | 원본 60초 전역 refresh를 유지하고, compact PoC stuck 완화를 짧은 재발행으로 보강                        |
...
구현 기준: W3X식 random attack-move refresh

- 일반 늑대의 기본 목표는 특정 `farm_core`가 아니라, 공격 구역 안의 랜덤 지점으로 향하는 attack-move order다.
- 원본 JASS의 `IliiIII`는 spawn/entry 시 `GetRandomLocInRect(IlIlI)`로 attack 명령을 발행하고, `IliiIlI -> IliiIiI`는 60초마다 전체 Player(10) 늑대에게 같은 방식의 attack 명령을 갱신한다.
- Phaser compact PoC는 원본 `IlIlI` 절대 좌표를 그대로 쓰지 않고, 현재 농장/플레이어 유닛 주변 local farm/play area rect에서 랜덤 지점을 뽑는다.
- 이동 중 acquire range 안의 농부/개/non-fence targetable building을 발견하면 focus/attack으로 전환한다.
- focus target이 잡힌 뒤 path가 끊긴 상태로 오래 멈추면 attack-move refresh로 빠져나오게 한다. focus가 있다는 이유만으로 stuck refresh를 영구 차단하지 않는다.
- focus 대상이 사거리 경계 근처인데 path가 없고 직접 공격도 안 되는 경우에는 짧은 direct approach fallback을 허용한다. 이는 Warsmash attack behavior가 사거리 밖 target을 향해 계속 접근하는 감각을 PoC에 반영한 것이다.
- fence는 직접 cleanup target이 아니다. fence 파괴는 현재 attack-move 경로가 막혔을 때 line/near/fallback blocker 공격으로 발생해야 한다.
- `combat_poc_result`는 farm 파괴 시 `farm_destroyed`로 기록한다. 남은 targetable blocker 수는 `remainingTargetableBlockerCount`로만 남긴다.
- stuck 상태가 오래 지속되는 compact PoC 문제는 `CHICKEN_FARM_BALANCE.pathing.wolfAi.attackMoveRefresh.stuckSec` 이후 랜덤 attack-move를 재발행해 완화한다. local random rect padding은 448px로 두어 농장 주변 전투성을 유지한다.
- post-objective cleanup은 현재 `CHICKEN_FARM_BALANCE.pathing.wolfAi.postObjectiveCleanup.enabled = false`로 둔다.
...
5. 완료: cleanup phase 중심 정책을 끄고, spawn/60초/stuck 기반 random attack-move refresh를 전투 AI에 연결했다.
