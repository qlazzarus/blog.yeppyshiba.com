...
const SUGGESTIONS = path.resolve(__dirname, '../src/data/internal-links-suggestions.json');
...
function readPromoCsv() {
  ...
  return lines.map(l => {
    const [pagePath, screenPageViews, averageSessionDuration_seconds, reason] = l.split(',');
    return { path: pagePath, views: Number(screenPageViews), avgSec: Number(averageSessionDuration_seconds), reason };
  });
}
...
function main() {
  ...
  if (suggestions && suggestions.topPost) {
    ...
    items.push({ path: suggestions.topPost, views: suggestions.topPostViews || 0, reason: 'top_post' });
    if (Array.isArray(suggestions.amplifyCandidates)) {
      suggestions.amplifyCandidates.slice(0, MAX_ITEMS).forEach(c => items.push({ path: c.path, views: c.views || 0, avgSec: c.avgSec || 0, reason: 'amplify' }));
    }
  } else {
    const promo = readPromoCsv();
    if (promo) {
      items = promo.slice(0, MAX_ITEMS).map(p => ({ path: p.path, views: p.views, avgSec: p.avgSec, reason: p.reason }));
    }
  }
  ...
  if (existing && existing.generatedAt) {
    ...
    if (ageMs < minMs) {
      console.log(`Skipping update: last generated ${Math.round(ageMs/3600000)}h ago (< ${MIN_AGE_HOURS}h)`);
      return;
    }
    const oldPaths = (existing.trending || []).map(i => i.path.replace(/\/+$/, ''));
  ...
  // write new trending
  const trending = filteredItems.slice(0, MAX_ITEMS).map(i => ({ path: i.path.replace(/\/+$/, ''), views: i.views || 0, avgSec: i.avgSec || 0, reason: i.reason || 'auto' }));
  writeOut(trending);
}
